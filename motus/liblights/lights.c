...
{
    return ((77*((state->color>>16)&0x00ff)) + (150*((state->color>>8)&0x00ff)) + (29*(state->color&0x00ff))) >> 8;
...
static int
set_speaker_light_locked(struct light_device_t* dev,
        struct light_state_t const* state, int battfix)
{
    int len;
    char buf[256];
    ...
    if (prevbattfix == 1 && colorRGB == 0x00ffffff)
        brightness = 0;

    prevbattfix = battfix;

#if 1 || !defined(morrison)
    ...
    len = sprintf(buf, "%d %d %d %d %d ramp %d %d %d %d", 
        brightness, onMS, offMS, state->flashOnMS1, state->flashOffMS1, 
        state->rampUp, state->rampDown,
        state->rampUp1, state->rampDown1);
#endif
...
{
    if ((&g_battery)->color != 0x00ffffff && is_lit(&g_battery)) {
        ...
        struct light_state_t const* state)
{
    ...
    g_battery = *state;
    if (g_haveTrackballLight) {
        set_speaker_light_locked(dev, state, 1);
    }
